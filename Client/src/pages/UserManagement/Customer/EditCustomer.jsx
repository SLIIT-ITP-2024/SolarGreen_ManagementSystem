...
const EditCustomer = ({ closeModal, customer, setLoader, loader }) => {
  ...
  const dob = customer?.dob ? new Date(customer.dob).toISOString().split('T')[0] : '';
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        const response = await axios.put(`http://localhost:3000/api/v1/customer-employee/edit-customer`, values);
